...
// content of the message in Mono.just(); then check if the message starts with our arbitrary prefix ("!" is this case).
...
// CommandFilter expects a Mono<Boolean> so we wrap this logic in Mono.just(). If the Mono returns true, then the
...
CommandFilter filter = context -> Mono.just(!context.getArgument().equals("ping"));
...
// "MyCommand" is simply the name to assign to this command. The name is used to uniquely identify commands and are
// useful for organization and coupling parent commands and subcommands (which will be explained later).
Command command = new CommandBuilder("MyCommand", executor).setArgumentFactory(factory).setFilters(filter).build();
